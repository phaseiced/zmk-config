...
static int pmw3360_async_init_fw_load_verify(const struct device *dev) {
     ...
     * REST on a side-looking ball sensor causes stuck/one-axis motion.
     */
#if IS_ENABLED(CONFIG_PMW3360_FORCE_AWAKE)
    err = reg_write(dev, PMW3360_REG_CONFIG2, 0x00);
    if (err) {
        LOG_ERR("Cannot disable REST modes (force-awake)");
    }
#else
...
static void set_interrupt(const struct device *dev, const bool en) {
    ...
    if (data->polling_mode || config->irq_gpio.port == NULL) {
        return;
    }
...
static void pmw3360_poll_callback(struct k_work *work) {
    ...
    pmw3360_report_data(dev);
    k_work_schedule(&data->poll_work, K_USEC(config->polling_interval_us));
}
...
static void pmw3360_async_init(struct k_work *work) {
    ...
    if (data->err) {
        ...
        if (data->async_init_step == ASYNC_INIT_STEP_COUNT) {
            ...
            LOG_INF("PMW3360 initialized (polling=%d)", data->polling_mode);
...
static int pmw3360_init(const struct device *dev) {
    ...
    k_work_init_delayable(&data->poll_work, pmw3360_poll_callback);
    ...
    /* Stock Keyball J2 has no MOTION pin — poll when irq-gpios is omitted. */
    data->polling_mode = (config->irq_gpio.port == NULL);
    if (!data->polling_mode) {
        ...
        LOG_INF("No irq-gpios; PMW3360 will poll every %u us", config->polling_interval_us);
    }
